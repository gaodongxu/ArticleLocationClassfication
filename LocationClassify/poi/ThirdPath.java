...
public class ThirdPath {
	public static void main(String[] args) {
		...
		String first_path_file = "D:\\data\\GraduationDesign\\poi_file\\second_path";
		String output_file = "D:\\data\\GraduationDesign\\poi_file\\third_path";
		check(all_path_file, first_path_file, output_file);
		System.out.println("OVRE!!!");
	}

	public static void check(String all_path_file, String second_path_file, String output_file) {
		BufferedWriter out = null;
		try {
			out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(output_file, false)));

			ArrayList<String> second_path_list = readList(second_path_file);
			HashMap<String, List<String>> all_path_map = readAllPathMap(all_path_file, second_path_list);
			for (String second_path : second_path_list) {
				out.write(second_path + "\n");
				for (String path : all_path_map.get(second_path)) {
					out.write(path + "\n");
				}
				out.flush();
			}
		...
		try {
			...
			while ((tempString = reader.readLine()) != null) {
				...
				for (String path : paths) {
					...
					// if (locs.length == 3 && tmpList[3].equals("main")) {
					if (locs.length == 3) {
						System.out.println(mainName + "\t" + path.split("->").length + "\t" + path);
						map.get(locs[1]).add(mainName);
						break;
					}
				}
			}
